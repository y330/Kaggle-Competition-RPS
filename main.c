...
be
used and modified
freely, as long as the * /
/ *original
author
credit is maintained.There
are
no
guarantees * /
/ *of
...
web
page
for future versions of this program:      * /
/ * * /
...
int
my_history[trials + 1], opp_history[trials + 1];
...
unfair
coin(bit)
with given probability of getting a 1 * /
...
int
biased_roshambo(double
prob_rock, double
prob_paper)
{
/ *roshambo
with given probabilities of rock, paper, or scissors * /
double
throw;
...
the
opponent
's most frequent choice */

int
i, rcount, pcount, scount;
...
/ *Don
't have to check over 1200 again because last entry is not 0. */
}
...
{2, 0, 2, 2, 2, 1, 0, 0, 1, 2, 2, 1, 2, 2, 2, 0, 2, 1, 2, 0, 0, 2, 1, 0, 2, 1, 0, 2, 2, 1, 1, 0, 0, 2, 2, 0, 0, 1, 0, 1,
 ...
 1, 2, 0, 0, 2, 1, 2, 1, 0, 0, 2, 2, 0, 2, 2, 2, 2, 1, 2, 0, 2, 2, 2, 2, 2, 2, 0, 1, 1, 0, 2, 2, 2, 2, 2, 0, 1, 1, 2,
 2};
...
int
antirotnbot() / * Anti - rotn
bot * /
{
/ *observes
rotations in opponent
's sequence,
exploits
max or min, whichever
...
by
opponent
's last move, but drift over time */
/ *max - EV = -0.50
ppt * /
...
Iocaine
Powder(
from: http: // ofb.net / ~egnor / iocaine.html)
...
of
strategies and
meta - strategies, entered in Darse
Billings
' excellent First International
RoShamBo
Programming
Competition.You
may
...
me
credit
for any derived works.I attempt here to
...
each
other
forever, " you say. But no; because of the
nature
of
RoShamBo, P
.3
...
.0! So
we
're only left with these three strategies, each of which will suggest
a
different
alternative.(This
may
...
opponent.If
P
' predicts that you will play rock, you would expect
your
opponent
to
play
...
theme
As
with P.1 and P.2, these represent "rotations" of the basic idea,
...
score, using the standard scoring scheme used by the tournament: +1
point if
the
strategy
would
...
this
algorithm as a
hedge; if someone is actually
able
to
predict and defeat
Iocaine
Powder
with any regularity, before long the random predictor
...
/ *last: opponent
's last move (-1 if none)
| guess: algorithm
's prediction of opponent'
s
next
move * /
static
...
struct
predict
pr_history[num_ages][3][2], pr_freq[num_ages][2];
struct
predict
pr_fixed, pr_random, pr_meta[num_ages];
struct
stats
stats[2];
};
...
const
int
last = (num > 0) ? opp_history[num]: -1;
const
int
guess = biased_roshambo(1.0 / 3.0, 1.0 / 3.0);
int
w, a, p;
...
thinks
you
'll play the strategy, and beat that.
3.
Assume
opponent
thinks
you
'll do (2), and beat that.
4.
Assume
opponent
plays
...
thinks
you
'll do (4), and beat that.
6.
Assume
opponent
thinks
you
'll do (5), and beat that.
...
beats
Rock,
if you assume the opponent thinks you'll do (3) then you'll
...
to
Dan
's original history mechanism and
Old
IP and Random(as a
fallback) for its set of base strategies.
...
means
it
's effectively checking to see how well it would do by playing to beat/lose
to
itself, and playing
that if it is better.I
've checked the effects of
this
"meta-ing", and after
the
first
...
strategies
doesn
't match the opponent'
s
play, then
Iocaine
's strategy becomes so subtle as to be effectively random.  If one
of
the
base
strategies
does
associate
with the opponent, then the meta-ing
does
no
good.
...
highest
scorer.Phasenbott
's metric would be more appropriate in a
non - zero
sum
Rock - Paper - Scissors
game
...
for
    ...
    there is now
    incentive
    for cooperation and no longer a single optimal
    strategy.Random
    scores
    an
    expected
    ...
    alternating
    wins,
    for 1 / 2.  A player wanting to do better than
    1 / 2
    would
    try to exploit the other player, but not enough that the other
    player
    detects
    that
    it
    's worthwhile to switch into Random mode.  The weak
    player
    scoring
    say
    2 / 5
    ...
    that
    it
    's being exploited by the stronger,
    but
    still
    go
    along
    with it as if it refused (by going Random) its score would
    drop
    to
    1 / 3.
    This in my
    ...
    * /
    ...
    static
    void
    jlm_history(jlmhistret * s)
    {
        int
    besta, bestb, bestc, i, j, num;
    / *a is both
    history, b is my
    history, c is opponent
    history. * /

    if (s->num == my_history[0])
    return;
    ...
    static
    arr4
    my_last, opp_last, opp_guess, my_guess;
    static
    int
    my_stats[4][3], opp_stats[4][3], my_ostats[4][3],
    opp_ostats[4][3];
    static
    int
    sy_last, spp_last, spp_guess, sy_guess;
    static
    int
    sy_stats[3], spp_stats[3], sy_ostats[3], spp_ostats[3];
    static
    jlmbot
    hb = {(fcn)
    jlmhist0, & h};
    ...
    for (h = 0; h < s->n; h++)
    {
        ...
        if (s->my_stats[h][i] > s->my_stats[hy_most][my_most])
            ...
            if (s->opp_stats[h][i] > s->opp_stats[hpp_most][opp_most])
                ...
                if (s->my_ostats[h][i] > s->my_ostats[hy_omost][my_omost])
                    ...
                    MegaHAL(
                    from: http: // ciips.ee.uwa.edu.au / ~hutch / puzzles / roshambo /)

                    MegaHAL, named in honour
                    of
                    a
                    conversation
                    simulator
                    ...
                    compile
                    it
                    with Darse's tournament program (available from the competition
                    ...
                    *shell.c(15
                    ...
                    is that
                    Darse
                    's competition provides an ideal test-bed for predictive
                    algorithms, or predictors.I
                    have
                    worked
                    with predictors for the last five
                    ...
                    interlacing of both our histories: x1, y1, x2, y2, ..., xn - 1, yn - 1, where
                    x
                    represents
                    our
                    opponent
                    's move, y represents our move, and our job is
                    to
                    predict
                    the
                    value
                    ...
                    1 * 0.40 - 1 * 0.15 = 0.25, and is therefore
                    chosen.
                ...
                providing
                us
                with a prediction about what's coming next. We need to select
                one
                of
                them.We
                do
                ...
                I
                apologise
                for any omissions or poor English, and blame that on the fact
                that
                it
                was
                written
            ...
            *NB: File
            displays
            best
            with tabstops set to three spaces.
            *
            *Name: MegaHAL( in honour
            of
            http: // ciips.ee.uwa.edu.au / ~hutch / hal /)
            *
            *Author: Jason
            Hutchens(hutch @ amristar.com.au)
            *
            *Purpose: Play
            the
            game
            ...
            *when
            ...
            contexts
            haven
            't yet been observed.  One of the
            *contexts is always
            guaranteed
            to
            make
            ...
            *trie.The
            ...
            the
            *model
            with the latest moves, and remove the statistics of
            *moves
            outside
            a
            sliding
            ...
            *Comment: Even
            though
            it
            's really messy, everything is done in this
            *one
            function
            to
            allow
            ...
            * must
            ...
            being
            used
            for prediction, HISTORY gives the
        ...
        * Returns: An
        integer
        which is positive if value1 < value2, negative if
        *value1 > value2, and zero if they
        're equal.  Various heuristics
        * are
        used
        to
        test
        ...
        *This
        ...
        complicated, but
        it
        's not.  Basically, given two nodes
        * in the
        trie, these
        heuristics
        decide
        ...
        *- If
        they
        're both being tried for the first time, choose the shortest.
        *- If
        they
        've both performed equally well, choose the longest.
        * /
        context1 = (CONTEXT *)
        value1;
        context2 = (CONTEXT *)
        ...
        * Did
        ...
        remembering
        past
        * strategies, and imroved
        the
        heuristics
        for sorting.Over 1000 tourneys
        ...
        * match.
        *
        * Revision 1.5  1999 / 09 / 13 16:51: 57
        hutch
        * The
        sliding
        window is working
        ...
        strategy
        * doesn
        't improve the performance of MegaHAL-Infinite on the standard
        * algorithms, but
        it
        will
        hopefully
        ...
        * Okay, experimentation
        with parameters has increased it's performance to
        ...
        * per match.
        *
        * Revision 0.3  1999 / 09 / 11 12:33: 54
        hutch \
        * Everything is working;
        ...
        standard
        bots
        * (performing at least twelve times better than the second best).I
        will
        * fine - tune
        the
        algorithm
        ...
        play
        * around
        with the heuristics before submitting.
        *
        * Revision 0.2  1999 / 09 / 11 11:40: 01
        hutch
        * The
        mechanism
        for selecting the best move has been finished, and the
        * model is working for a NULL context.Now we need to expand it to the
        * infinite context.
        *
        * Revision 0.1  1999 / 09 / 11 05:58: 29
        hutch
        * Initial
        revision
        * /
        ...
        's some more feedback & personal info for you.  Feel free to include
        any
        of
        it
        at
        ...
        after a
        while ties for most likely opponent choice were broken by choosing
        randomly.
        ...
        I
        'd misunderstood and thought that reverting to random behavior even as a
        "bailout"
        measure
        was
        considered
        ...
        would
        be
        "if I'm losing hideously, then start doing the
        opposite
        of
        whatever
        my
        ...
        fiddled
        off & on
        with my program for about 5 days.It went through quite
        a
        few
        iterations, and I
        played
        ...
        on
        1830(
        from Avalon Hill) and Master
        of
        Orion
        2(
        from Microprose), doing
        AI
        for both.I plan to work on AI for Go one of
        these
        days.
        * /
        ...
        return (1 + biased_roshambo(n_votes[0] / (double)(n_votes[0] + n_votes[1]
                                                          + n_votes[2]),
                                    n_votes[1] / (double)(n_votes[0] + n_votes[1] + n_votes[2]))) % 3;
        }
        / *russrocker4 * /
           / ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** /
        ...
        EV_TTL
        if (ttl <= 0)
            return (biased_roshambo((double) 1.0 / 3, (double)
            1.0 / 3 ) );
            ...
            *Initialize
            ...
            for (i = 0, ix = 3; i < CSIZE; i++, ix *= 3)
                {
                    myh[i] = malloc(ix * sizeof(short
                int) *3 );
                oph[i] = malloc(ix * sizeof(short
                int) *3 );
                }
                }
                ...
                for (i = 0, ix = 3; i < CSIZE; i++, ix *= 3)
                    {
                        bzero(myh[i], ix * sizeof(short
                    int) *3 );
                    bzero(oph[i], ix * sizeof(short
                    int) *3 );
                    }
                    }
                    ...
                    * /
                    ...
                    for (i = 0; i < 3; i++)
                        ...
                        * /
                        ...
                        not going
                        well
                        with our predicitons, make * /
                        / * random moves for a
                        while to confuse the opponent * /
                        if (score <= -10 & & gorandom == 0)
                            ...
                            r / p / s
                            combinations
                            for the two players, counts are kept
                            ...
                            simple
                            predictor, but
                            for both players.It can therefore counter an
                            opponent
                            using
                            a
                            similar
                            ...
                            count
                            set
                            with the highest score.If no count set shows a
                            positive
                            score, it
                            plays
                            at
                            ...
                            * /
                            ...
                            int
                            i, j, k, l, m, m1, o, o1, s, id;
                            double
                            q, qi, qj, qk, ql, qd;
                            int
                            history_length = my_history[0];
                            m = 0;
                            m1 = 0;
                            o = 0;
                            o1 = 0; / *-db * /
                            ...
                            history_length = my_history[0];
                            int
                            i, j, k, l, m, m1, o, o1, s, mr, mp, ms, mb;
                            double
                            q, qi, qj, qk, ql;
                            if (history_length == 0)
                            ...
                            if (history_length > 1)
                                ...
                                / * gather
                                stats
                                for counts of related events, NHIST back * /
                                ...
                                / *Only
                                with enough data try to predict! * /
                                pos = opp_history[0];
                                rsb = opp_history[pos];
                            ...
                            float
                            bail_l_min, bail_l_max, bail_l_diff;
                            ...
                            i, j;
                            int
                            opp_earlier, my_earlier, opp_last, my_last;
                            float
                            best, pred_r, pred_p, pred_s;

                            int
                            filter_opp, filter_me, filter_lag;
                            ...
                            {
                            int
                            mymove = my_history[my_history[0]];
                            ...
                            for (i = 0; i < cur->ivar[0]; ++i)
                                ...
                                for (i = 9; i < sc; ++i)
                                    if (s[i].score < worstscore)
                                        ...
                                        unsigned
                                        int
                                        c_trseqrpt, c_trresrpt, f_notwoseq, f_nothreeseq,
                                        f_bigom, f_okbedumb;
                                        static
                                        int
                                        f_cpyoffset = 0, om = 0;
                                        ...
                                        if (my_history[0] == 0)
                                            ...
                                            om = oc[0] = oc[1] = oc[2] = pcts[0] = pcts[1] = pcts[2] = tots[0] = tots[
                                                1] =
                                        ...
                                        on
                                        an
                                        ACT - R(http: // act.psy.cmu.edu) model
                                        that
                                        plays
                                        RoShamBo.The
                                        model
                                        ...
                                        * /
                                        ...
                                        {
                                        double
                                        frequencies[3], score, p, best_score = maxrandom;

                                        int
                                        back1 = 0, back2 = 0, i, winner, index = my_history[0];
                                        ...
                                        if (index >= 2)
                                            ...
                                            for (i=0;i < index;i++)
                                                ...
                                                if (best_score == maxrandom | | score > best_score)
                                                    ...
                                                    windowSize = 5;
                                                    / *int
                                                    markovLength = 9; * / / *3 ^ size * /
                                                    static
                                                    int
                                                    wins, losses;
                                                    static
                                                    double
                                                    percentWins, percentLosses, margin;

                                                    static
                                                    double
                                                    MarkovChain[markovLength][3];
                                                    static
                                                    int
                                                    Markovuse[markovLength];
                                                    static
                                                    int
                                                    MarkovTally[markovLength][3];

                                                    retval = 0; / *-db * /
                                                    if (my_history[
                                                            0] == 0) / * if we're just starting, init the array. */
                                                    {
                                                    ...
                                                    percentWins=percentLosses=margin=0;
                                                        ...
                                                        last
                                                        turn * /
                                                        if ((opp_history[opp_history[0]] + 1) % 3 == my_history[
                                                            my_history[0]])
                                                        wins + +;
                                                        else if ((opp_history[opp_history[0]] + 2) % 3 == my_history[
                                                            my_history[0]])
                                                        losses++;
                                                    ...
                                                    if (opp_history[
                                                            0] > windowSize) / * if we're past P1.. remember, P1=P0 */
                                                    {
                                                        markovindex = 0;
                                                    for (i=windowSize;i >= 1;i--)
                                                    markovindex+= ((i == 1)?
                                                    (opp_history[opp_history[0]-i]):(
                                                                opp_history[opp_history[0] - i] * 3));

                                                    / *now if we
                                                    haven
                                                    't used the row before, zero it and put a one in the */
                                                    / * right
                                                    place * /
                                                    ...
                                                    / * if we
                                                    're more that 60% behind or ahead, bail. also, if we haven'
                                                    t
                                                    done * /
                                                    / *even
                                                    one
                                                    move, don
                                                    't use the markov chain if we don'
                                                    t
                                                    have
                                                    a
                                                    previous * /
                                                    ...
                                                    if ((my_history[0] <= windowSize) / * | | (score < -50) * /)
                                                        ...
                                                        / * if we
                                                        didn
                                                        't bail, we use the markov chain */
                                                        / *
                                                        for now use random * /
                                                        / * retval=(biased_roshambo(0.33333, 0.33333)); * /
                                                    ...
                                                    if (
                                                    !(newprob < cumprob)) / * test to make sure we don't have floating point error */
                                                    retval=0; / * ((2+1) % 3) * /
                                                    }
                                                    ...
                                                    ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** /
                                                    ...
                                                    oL = opp_history[ng];
                                                    int
                                                    oL1 = opp_history[ng - 1];
                                                    int
                                                    oL2 = opp_history[ng - 2];
                                                    int
                                                    oL3 = opp_history[ng - 3];
                                                    ...
                                                    ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** /
                                                    ...
                                                    static
                                                    zq_node_t
                                                    zq_root_node, *zq_root = NULL;
                                                    static
                                                    int
                                                    zq_num_nodeblocks, zq_patt_length = 9;
                                                    ...
                                                    static
                                                    void
                                                    zq_kill_children(zq_node_t * node)
                                                    {
                                                    int
                                                    i;
                                                    ...
                                                    the
                                                    tree
                                                    for last zq_patt_length moves, last 6, ..., last move * /
                                                    ...
                                                    static
                                                    int
                                                    count[3][3][3]; / *[Idid][Itdid][Itdoes]; * /
                                                    static
                                                    int
                                                    lastTime[3][3][1]; / *[Idid][Itdid][Itdid] * /
                                                    static
                                                    int
                                                    score, goingbad;
                                                    ...
                                                    score = 0;
                                                    goingbad = 0;
                                                    return (biased_roshambo(0.33, 0.33)); / *Be
                                                    optimal
                                                    first * /
                                                    }

                                                    if (my_history[0] < LMIN2) {
                                                    if ((opp_history[opp_history[0]] - my_history[
                                                        my_history[0]] == 1) | |
                                                            (opp_history[opp_history[0]] - my_history[
                                                                my_history[0]] == -2)) {
                                                    score --;
                                                    ...
                                                    result
                                                    information * /
                                                    pCount = count[my_history[my_history[0] - 1]][
                                                    ...
                                                    score - -;
                                                            ...
                                                            opp_history[opp_history[0]] != my_history[my_history[0]]) {
                                                    score + +;
                                                    }
                                                    ...
                                                    return (biased_roshambo(0.333, 0.333)); / *better
                                                    be
                                                    optimal
                                                    then * /
                                                    }

                                                    pLast = lastTime[my_history[my_history[0] - 1]][
                                                        opp_history[opp_history[0] - 1]];
                                                    ...
                                                    do
                                                    next * /
                                                    pCount = count[my_history[my_history[0]]][
                                                        opp_history[opp_history[0]]];
                                                    ...
                                                    last
                                                    time * /
                                                    pLast = lastTime[my_history[my_history[0]]][
                                                        opp_history[opp_history[0]]];

                                                    if ((pCount[rock] > pCount[paper]) & & (
                                                            pCount[rock] > pCount[scissors])) {
                                                    pred = rock;
                                                    choice = paper;
                                                    ...
                                                    my
                                                    experiences
                                                    with Robot Battle (RB) where the Muli-mode
                                                    ...
                                                    * /
                                                    ...
                                                    RollingAverage * RollingAverage_new(int
                                                    size);
                                                    void
                                                    RollingAverage_delete(RollingAverage * avg);
                                                    float
                                                    RollingAverage_Add(RollingAverage * avg, float
                                                    element);
                                                    float
                                                    ...
                                                    extern
                                                    Strategy * Strategy_new(int(*function)(), int
                                                    length);
                                                    extern
                                                    void
                                                    ...
                                                    if (FirstTrial())
                                                        ...
                                                        for (i = 0; i < strategy_count; i++)
                                                            {
                                                                float
                                                            success = RollingAverage_Add(strategies[i]->success,
                                                                                                        Score(
                                                                                                            opp_history[
                                                                                                                0],
                                                                                                            strategies[
                                                                                                                i]->lastmove));
                                                            int
                                                            move = Strategy_Move(strategies[i]);
                                                        ...
                                                        RollingAverage *
                                                        RollingAverage_new(int
                                                        size)
                                                        {
                                                            int
                                                        i;
                                                        RollingAverage * avg = (RollingAverage *)
                                                        malloc(sizeof(RollingAverage));
                                                        avg->total = 0.0
                                                        f;
                                                        avg->count = 0;
                                                        avg->next = 0;
                                                        ...
                                                        if (avg->count > avg->size)
                                                            ...
                                                            Strategy *
                                                            Strategy_new(int(*function)(), int
                                                            length)
                                                            {
                                                                Strategy * result = (Strategy *)
                                                            malloc(sizeof(Strategy));
                                                            ...
                                                            for (i = 0; i < 4; i++)
                                                                ...
                                                                for (i = 0; i < 16; i++)
                                                                    ...
                                                                    for (i = 0; i < 16; i++)
                                                                        ...
                                                                        base and weirdo * /
                                                                        trial = my_history[0];
                                                                        if (trial > 0 & & my_history[trial] == rock & &
                                                                                opp_history[trial] == paper)
                                                                            base += 2;
                                                                        if (trial > 0 & & my_history[trial] == paper & &
                                                                                opp_history[
                                                                                    trial] == scissors) base += 2;
                                                                        if (trial > 0 & & my_history[
                                                                            trial] == scissors & & opp_history[
                                                                            trial] == rock)
                                                                            base += 2;
                                                                        if (trial > 0 & & my_history[trial] ==
                                                                                opp_history[trial])     base += 1;
                                                                        if (base < 0) base=0;
                                                                        if (
                                                                                base > 9) {base=0;weirdo=(weirdo+1 % 3);} / * 6, 9, 12, 15 are good BASE numbers.* /
                                                                        ...
                                                                        / *maintain
                                                                        stats
                                                                        with static variables to avoid re-scanning the
                                                                        history
                                                                        array * /
                                                                        static
                                                                        struct
                                                                        ...
                                                                        r, p, s;} oc, mc;
                                                                        int
                                                                        opp_last, opp_prev, my_last, my_prev, myfreq, i;
                                                                        ...
                                                                        return (paper);}
                                                                                ...
                                                                                oc.p - oc.r - oc.s) > 0) {return (scissors);}
                                                                        else if ( (oc.s - oc.p - oc.r) > 0) {return (rock);}

                                                                        / * beat rotate * /
                                                                        i = opp_history[0]-50;
                                                                        if (i < 0) i=1;
                                                                        while ((i < opp_history[0]) & & (
                                                                                (opp_history[i] + 1) % 3 == opp_history[
                                                                            i + 1]))
                                                                            i + +;
                                                                        if (i == opp_history[0]) {
                                                                        ...
                                                                        very
                                                                        much
                                                                        for your email about the results from the
                                                                        ...
                                                                        Inocencio
                                                                        's results were pretty bad, as it deserved.  No surprise
                                                                        after
                                                                        reading
                                                                        the
                                                                        descriptions
                                                                        ...
                                                                        * /
                                                                        ...
                                                                        / *Entrant: Bugbrain(12) and Knucklehead(52)
                                                                        Sunir
                                                                        Shah(Can)
                                                                        ...
                                                                        code
                                                                        to
                                                                        Bugbrain and Crazybot in comp.ai.games.
                                                                            I
                                                                        hereby
                                                                        donate
                                                                        the
                                                                        ...
                                                                        * /
                                                                        ...
                                                                        if (!my_history[0])
                                                                            {
                                                                                pTracker->iCurrentPlayer = (pTracker
                                                                                                            ->iCurrentPlayer + 1) % players;
                                                                            }
                                                                            ...
                                                                            * /
                                                                            ...
                                                                            GCC
                                                                            warning
                                                                            for above commented code * /
                                                                            static
                                                                            int
                                                                            bPlayerArrayZeroed = 0;

                                                                            static
                                                                            sunPLAYERTRACKER
                                                                            Tracker = {0};

                                                                            sunCRAZYBOT * pPlayer = & aPlayers[
                                                                                sunTrackPlayer( & Tracker)];

                                                                            if (!bPlayerArrayZeroed)
                                                                                {
                                                                                    bPlayerArrayZeroed = 1;
                                                                                memset(aPlayers, 0, sizeof
                                                                                aPlayers );
                                                                                }
                                                                                # else
                                                                                static
                                                                                ...
                                                                                if (my_history[0])
                                                                                    {
                                                                                        int
                                                                                    iResult = sunRoshamboComparison(
                                                                                        sunMYLASTTURN, sunOPPLASTTURN);
                                                                                ...
                                                                                if (flip_biased_coin(pPlayer
                                                                                ->dShuffleProbability) )
                                                                                sunShuffleCrazybotPlayer(pPlayer);

                                                                                return pPlayer->iLastTurn = pPlayer->
                                                                                aiTransform[pPlayer->iLastTurn];
                                                                                }
                                                                                ...
                                                                                ** [ways of arranging opponent
                                                                                ...
                                                                                the
                                                                                player
                                                                                's matrix to initially random probabilities,
                                                                                ** taking
                                                                                care
                                                                                to
                                                                                ensure
                                                                                ...
                                                                                ** (my last turn, opponents last turn)
                                                                                ...
                                                                                static
                                                                                double(*apfnAttenuations[])(double
                                                                                dValue) = {
                                                                                ...
                                                                                dDelta;
                                                                                int
                                                                                iNextProbability, iOtherProbability;
                                                                                double
                                                                                dNextProbability, dOtherProbability;
                                                                                ...
                                                                                if (!bPlayerArrayZeroed)
                                                                                    ...
                                                                                    's vector and result */
                                                                                    dDelta = pPlayer->
                                                                                    adProbabilities[sunMYPREVTURN(2)][
                                                                                        sunOPPPREVTURN(2)][
                                                                                        sunMYLASTTURN];
                                                                                    ...
                                                                                    iNextProbability = (
                                                                                                                   sunMYLASTTURN + 1) % 3;
                                                                                    iOtherProbability = (
                                                                                                                    iNextProbability + 1) % 3;

                                                                                    dNextProbability = pPlayer->
                                                                                    adProbabilities[sunMYPREVTURN(2)][
                                                                                        sunOPPPREVTURN(2)][
                                                                                        iNextProbability];
                                                                                    dOtherProbability = pPlayer->
                                                                                    adProbabilities[sunMYPREVTURN(2)][
                                                                                        sunOPPPREVTURN(2)][
                                                                                        iOtherProbability];
                                                                                    ...
                                                                                    * /
                                                                                    dDelta = dDelta * dNextProbability / (
                                                                                                dNextProbability + dOtherProbability);
                                                                                    ...
                                                                                    return biased_roshambo(
                                                                                        pPlayer->
                                                                                    adProbabilities[sunMYLASTTURN][
                                                                                        sunOPPLASTTURN][rock],
                                                                                    pPlayer->
                                                                                    adProbabilities[sunMYLASTTURN][
                                                                                        sunOPPLASTTURN][paper] );
                                                                                    }
                                                                                    ...
                                                                                    To give credit, where credit was due:  Frank
                                                                                    Hill and T.J.Walls
                                                                                    (also
                                                                                    of
                                                                                    Reliable
                                                                                    ...
                                                                                    *karma = & turn;
                                                                                    ...
                                                                                    eleven. * /
                                                                                    for (magic_bacon
                                                                                    = 0; magic_bacon < turn; magic_bacon++) THELOOPSTARTS
                                                                                        eleven = 1;
                                                                                        if ((*(*karma + magic_bacon)) !=
                                                                                                arr[
                                                                                                    magic_bacon]) THEIFSTARTS
                                                                                        eleven = 0;
                                                                                        break;
                                                                                    }
                                                                                    }
                                                                                    ...
                                                                                    karma
                                                                                    ratio * / (0[karma]) += (fresh
                                                                                                             bananas * karma / fresh grapes * karma);
                                                                                    }

                                                                                    / * return the...whatever
                                                                                    the
                                                                                    heck
                                                                                    this is.* / recycle(char *) * karma;
                                                                                    }

                                                                                    int
                                                                                    RST_ULTIMATE_ANALYZER_FUNCTION()
                                                                                    THEFUNCTIONSTARTS
                                                                                    int
                                                                                    turn[1];
                                                                                    ...
                                                                                    *turn = opp_history[0];
                                                                                    ...
                                                                                    if (*turn < trials - 2)
                                                                                        return libra ? callback(): be_good_for
                                                                                    GOD
                                                                                    sake;
                                                                                    ...
                                                                                    throw. * /
                                                                                    good_hand = (int *)
                                                                                    find_goodkarma(my_history);
                                                                                    bad_hand = (int *)
                                                                                    find_goodkarma(opp_history);

                                                                                    if (
                                                                                            cancer == 1) return * (int * )"ROCK";
                                                                                    if (
                                                                                            scorpio == 1) return * (int * )"SCISSORS";
                                                                                    if (!(( scorpio != 0) | | (
                                                                                            cancer != 1)) | | !((cancer != 0) | |
                                                                                    ...
                                                                                    Values * /
                                                                                    for (i
                                                                                    = 1; i <= * turn; i++) THELOOPSTARTS
                                                                                        if (good_hand[i] == bad_hand[
                                                                                            i]) libra++;
                                                                                        else if ( ((good_hand[i] - bad_hand[i] +3) GOD sake) == 1) cancer++;
                                                                                        else scorpio + +;

                                                                                        good_hand[i] = (bad_hand[i] + 1)
                                                                                        GOD
                                                                                        sake;
                                                                                    }

                                                                                    if (
                                                                                            bad_hand > good_hand) THEIFSTARTS
                                                                                    i = cancer;
                                                                                    cancer = scorpio;
                                                                                    scorpio = i;
                                                                                    democrats = 2;
                                                                                    ...
                                                                                    pizza[2] = 0;
                                                                                    pizza[
                                                                                        3 - democrats] = cancer + scorpio + libra;
                                                                                    democrats = 0;

                                                                                    recycle(pizza[democrats] + rotten
                                                                                    bananas
                                                                                    good_hand) GOD
                                                                                    sake;
                                                                                    }
                                                                                    ...
                                                                                    void
                                                                                    Init_Player_Table(Player_Table
                                                                                    crosstable[players + 1])
                                                                                    {
                                                                                        int
                                                                                    i, j;
                                                                                    ...
                                                                                    strcpy(crosstable[i].name,
                                                                                           "Psychic Friends N");
                                                                                    crosstable[
                                                                                        i].pname = RST_ULTIMATE_ANALYZER_FUNCTION;
                                                                                    ...
                                                                                    int
                                                                                    i, j, p1, p2, p1total, p2total, ties;
                                                                                    int
                                                                                    p1hist[trials + 1], p2hist[
                                                                                        trials + 1];
                                                                                    ...
                                                                                    if ( (p1 < 0) | | (p1 > 2) ) {
                                                                                    printf(
                                                                                        "Error: return value out of range.\n");
                                                                                    p1 = (
                                                                                                     p1 % 3 + 3) % 3; / *note: -5 % 3 = -2, not 1 * /
                                                                                    }

                                                                                    memcpy(opp_history, p1hist,
                                                                                           sizeof(int) * (trials + 1));
                                                                                    memcpy(my_history, p2hist,
                                                                                           sizeof(int) * (trials + 1));
                                                                                    ...
                                                                                    if (
                                                                                    verbose1) {printf(" p1 = %d, p2 = %d", p1, p2);}
                                                                                    if (p1 == p2) {
                                                                                    ties + +;
                                                                                    ...
                                                                                    p1total + +;
                                                                                    if (
                                                                                    verbose1) {printf(" p1 wins!\n");}}
                                                                                    else if ( (p2-p1 == 1) | | (p2-p1 == -2) ) {
                                                                                    p2total + +;
                                                                                    if (
                                                                                    verbose1) {printf(" p2 wins!\n");}}
                                                                                    else printf("Error: should not be reached.\n");
                                                                                    }
                                                                                    if (verbose2) {
                                                                                    ...
                                                                                    void
                                                                                    Print_Sorted_Results(Player_Table
                                                                                    crosstable[players + 1])
                                                                                    {
                                                                                    int
                                                                                    i, j, max, swap;
                                                                                    ...
                                                                                    void
                                                                                    Print_Scaled_Results(Player_Table
                                                                                    crosstable[players + 1])
                                                                                    {
                                                                                    int
                                                                                    i, j, N;
                                                                                    ...
                                                                                    void
                                                                                    Print_MSorted_Results(Player_Table
                                                                                    crosstable[players + 1])
                                                                                    {
                                                                                    int
                                                                                    i, j, max, swap;
                                                                                    ...
                                                                                    void
                                                                                    Print_Match_Results(Player_Table
                                                                                    crosstable[players + 1])
                                                                                    {
                                                                                    int
                                                                                    i, j, N;
                                                                                    ...
                                                                                    if (
                                                                                    verbose2) {Print_T_Results (crosstable);}
                                                                                    }
                                                                                    ...
                                                                                    printf(
                                                                                        " Playing %d tournaments with %d trials per match...\n\n",
                                                                                        tourneys, trials);
                                                                                    ...
                                                                                    printf(
                                                                                        " Final results (draw value = %d):\n",
                                                                                        g_drawn);
                                                                                    Print_Scaled_Results(crosstable);
                                                                                    Print_Match_Results(crosstable);
                                                                                    / *add
                                                                                    one
                                                                                    for luck(compare to last iteration)
                                                                                    g_drawn + +;
                                                                                    Print_Match_Results(crosstable);
                                                                                    * /
                                                                                    return (0);
